...
class Parser {
  ...
  private function getTVShowList($query) {
    $shows = $this->getJSON(
      'http://api.trakt.tv/search/shows.json/',
       array($query)
    );
  ...
  private function getSeasons($showid) {
    $seasons = $this->getJSON(
      'http://api.trakt.tv/show/seasons.json/',
      array($showid)
    );

    return array('showId' => $showid, 'seasons' => array_keys($seasons));
  }
  ...
  private function getEpisodes($showid, $season) {
    $episodes = $this->getJSON(
      'http://api.trakt.tv/show/season.json/',
      array($showid,$season)
    );
